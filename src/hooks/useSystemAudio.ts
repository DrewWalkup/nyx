...
export function useSystemAudio() {
	...
	useEffect(() => {
		...
		const setupContinuousListeners = async () => {
			try {
				...
				discardedUnlisten = await listen(
					"speech-discarded",
					(event) => {
				...
				console.error(
					"Failed to setup continuous recording listeners:",
					err,
				);
			}
		};
				...
				speechUnlisten = await listen(
					...
					async (event) => {
						try {
							...
							if (!selectedSttProvider.provider && !useNyxAPI) {
		...
		if (lastTranscription && lastTranscription.trim()) {
			...
			if (!lastMessage || lastMessage.content !== lastTranscription) {
				...
				setConversation((prev) => ({
					...
					title:
						prev.title ||
						generateConversationTitle(lastTranscription),
				}));
			}
		}
	...
	const processWithAI = useCallback(
		async (
			...
			try {
				...
				const useNyxAPI = await shouldUseNyxAPI();
				if (!selectedAIProvider.provider && !useNyxAPI) {
				...
				if (fullResponse) {
					...
					setConversation((prev) => ({
						...
						title:
							prev.title ||
							generateConversationTitle(transcription),
					}));
				}
			} catch (err) {
	...
	const startCapture = useCallback(async () => {
		try {
			...
			const hasAccess = await invoke<boolean>(
	...
	const handleSetup = useCallback(async () => {
		try {
			...
			if (hasAccess) {
				...
				setSetupRequired(true);
				setError(
					"Permission not granted. Please try the manual steps.",
				);
			}
		} catch (err) {
			setError(
				"Failed to request access. Please try the manual steps below.",
			);
			setSetupRequired(true);
		}
	}, [startCapture]);
	...
	useEffect(() => {
		...
		saveTimeoutRef.current = setTimeout(async () => {
			...
			try {
				...
				console.error(
					"Failed to save system audio conversation:",
					error,
				);
			} finally {
