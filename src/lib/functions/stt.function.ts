...
export async function fetchSTT(params: STTParams): Promise<STTResult> {
	try {
		...
		// Check if we should use Nyx API instead
		const useNyxAPI = await shouldUseNyxAPI();
		if (useNyxAPI) {
			return await fetchNyxSTT(audio);
		}
		...
		const formData = deepVariableReplacer(
			curlJson.form || {},
			allVariables,
		);
		...
		const replacedParams = deepVariableReplacer(
			decodedParams,
			allVariables,
		);
			...
			for (const [key, val] of Object.entries(formData)) {
				...
				if (!isNaN(parseInt(key, 10))) {
					...
					if (
						!formValue ||
						headerKeys.includes(
							formKey.toUpperCase().replace(/[-_]/g, ""),
						)
					)
						continue;
